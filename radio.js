...
ReadStream.prototype.onData = function(chunk) {
  if (this.metaint && this.counter + chunk.length >= this.metaint) {
    var audioEnd = this.metaint - this.counter -1 ;
    if (this.counter < this.metaint) {
      ...
      this.connection.emit("data", audioChunk);
    }
    ...
    if (audioEnd + 1 < chunk.length) {
      var remainder = chunk.slice(audioEnd + 1, chunk.length);
      this.connection.emit("data", remainder);
    }      
  } else {
    this.emit("data", chunk);
    //console.error(this.counter+"+"+chunk.length+"= " + (this.counter + chunk.length));
...
ReadStream.prototype.onDataHeader = function(chunk) {
  ...
  for (var i=0, l=Math.min(this.headerBuffer.length-3, 8192); i<l; i++) {
    ...
    if (this.headerBuffer[i] == R && this.headerBuffer[i+1] == N && this.headerBuffer[i+2] == R && this.headerBuffer[i+3] == N) {
      ...
      var leftover = this.headerBuffer.slice(i+1, this.headerBuffer.length);
      ...
      // inspect them, and set up `ffmpeg` based on the 'conent-type' perhaps
      this.emit("connect", this);

      // If there was any bytes leftover after the header, then it's audio data that should
...
ReadStream.prototype.onMetaLengthByte = function(chunk) {
  if (this.metaByteBuffer) {
    this.metaByteBuffer = exports.appendBuffer(this.metaByteBuffer, chunk);
  } else {
    this.metaByteBuffer = chunk;
  }
  if (this.metaByteBuffer.length > 1) {
    var metaByte = this.metaByteBuffer[0];
    if (metaByte === 0 || this.metaByteBuffer[1] === 83) {
      ...
      var remains = this.metaByteBuffer.slice(1, this.metaByteBuffer.length);
      this.metaByteBuffer = null;
      this.connection.emit("data", remains);
    } else {
      this.connection.removeListener("data", this.bindedOnMetaLengthByte);
      this.connection.addListener("data", this.bindedOnData);
      this.connection.emit("data", this.metaByteBuffer.slice(0,1));
      var remains = this.metaByteBuffer.slice(1, this.metaByteBuffer.length);
      this.metaByteBuffer = null;
      this.metaLength = null;
      this.connection.removeListener("data", this.bindedOnData);
...
ReadStream.prototype.parseHeaders = function() {
  ...
  //console.error(this.headerString);
