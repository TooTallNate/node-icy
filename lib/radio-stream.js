...
require("util").inherits(ReadStream, require("stream").Stream);
...
ReadStream.prototype.onDataHeader = function(chunk) {
  ...
  for (var i=0, l=Math.min(this.headerBuffer.length-3, 8192); i<l; i++) {
    ...
    if (this.headerBuffer[i] == R && this.headerBuffer[i+1] == N && this.headerBuffer[i+2] == R && this.headerBuffer[i+3] == N) {
      ...
      // If there are any bytes leftover after the header, then it's audio
      // data that should be handled by the new 'onData' callback.
...
ReadStream.prototype.parseHeaders = function() {
  ...
  var firstLine = this.headerString.substring(0, this.headerString.indexOf("\r\n")).split(" ");
  ...
  for (var i=0, l=headers.length; i<l; i++) {
    ...
    var field = header.substring(0, firstColon);
