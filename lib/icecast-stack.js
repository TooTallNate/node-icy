...
IcecastWriteStack.prototype.queueMetadata = function(metadata) {
  if (typeof(metadata) == 'string' && !streamTitleRegExp.test(metadata)) {
    // Ensure a 'StreamTitle' in somewhere in the string.
  ...
  var metaBuf = new Buffer(metaLength * META_BLOCK_SIZE);
  metaBuf[0] = metaLength;
  var end = metaBuf.write(metadata, 1) + 1;
  if (end < metaBuf.length) {
    for (var l = metaBuf.length - end; end<l; end++) {
      metaBuf[end] = 0;
    }
  }
